...
class ProxyUpdater:
    ...
    def fetch_proxy_servers(self) -> List[Dict]:
        ...
        for source in self.sources:
            try:
                ...
                self.logger.info(f"Fetched {len(servers)} servers from {source['name']}")
    ...
    def validate_servers(self, servers: List[Dict]) -> List[Dict]:
        ...
        for server in servers:
            if self._test_server_connectivity(server):
                ...
                self.logger.warning(f"Server {server['host']}:{server['port']} is not responding")
    ...
    def generate_configurations(self, servers: List[Dict]) -> None:
        ...
        try:
            ...
            self._save_config("../configs/singbox.json", json.dumps(singbox_config, indent=2))

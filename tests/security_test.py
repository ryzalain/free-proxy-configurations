...
class SecurityTester:
    ...
    def __init__(self, proxy_config: Dict[str, str] = None, timeout: int = 10):
        ...
        :param proxy_config: Dict for requests, e.g.,
                            {'http': 'socks5://...', 'https': 'socks5://...'}
    ...
    def test_ip_leak(self) -> bool:
        ...
        try:
            real_ip = requests.get(
                "https://httpbin.org/ip", timeout=self.timeout
            ).json()["origin"]
            proxy_ip_res = requests.get(
                "https://httpbin.org/ip", proxies=self.proxy_config,
            ...
            if is_leak:
                print(f"   └── ❌ IP LEAK DETECTED: "
                      f"Your real IP ({real_ip}) is exposed!")
                return False
            else:
                print(f"   └── ✅ IP successfully masked: "
                      f"{real_ip} -> {proxy_ip}")
                return True
    ...
    def test_dns_leak(self) -> bool:
        ...
        try:
            response = requests.get(
                "https://1.1.1.1/cdn-cgi/trace", proxies=self.proxy_config,
                timeout=self.timeout
            )
            trace_data = {
            ...
            if "loc" in trace_data:
                print(f"   └── ✅ DNS appears to be routed correctly. "
                      f"Location: {trace_data['loc']}")
                return True
            else:
    ...
    def test_ipv6_leak(self) -> bool:
        ...
        try:
            response = requests.get(
                "https://ipv6.icanhazip.com", proxies=self.proxy_config,
                timeout=self.timeout
            )
            if response.ok and response.text.strip():
                print(f"   └── ⚠️ IPv6 connection detected: "
